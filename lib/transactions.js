...
exports.createGeneration = (rpcData, blockReward, feeReward, recipients, poolAddress, poolHex, coin, masternodeReward, masternodePayee, masternodePayment, zelnodeBasicAddress, zelnodeBasicAmount, zelnodeSuperAddress, zelnodeSuperAmount, zelnodeBamfAddress, zelnodeBamfAmount) => {
    ...
    if(coin.easyTXGeneration === false || coin.easyTXGeneration === undefined)
    {
    ...
    {
        txb.addOutput(new Buffer(rpcData.treasury.scriptPubKey, 'hex'), rpcData.treasury.amount); 
    }
    ...
    {
        if(rpcData.masternode.script !== undefined) // if it's undefined, then there is currently no masternode to pay
            txb.addOutput(new Buffer(rpcData.masternode.script, 'hex'), rpcData.masternode.amount); 
    }
