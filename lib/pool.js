...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function OnBlockchainSynced(syncedCallback) {
        ...
        {
            checkSynced = function (displayNotSynced) {
                _this.daemon.cmd('getblocktemplate', [callConfig, options.coin.MultiAlgoString], function (results) {
    ...
    function GetBlockTemplate(callback) {
        ...
        function getBlockTemplate(subsidy) {
            ...
            if(options.coin.isMultiAlgo)
            {
                ...
                [callConfig, options.coin.MultiAlgoString],
                function (result) {
                        if (result.error) {
                            ...
                            // If we mine GLT, we don't need this whole stuff here, skip it and just set the miner reward.
                            if(options.coin.easyTXGeneration)
                            {
                                result.response.miner = result.coinbasevalue; // Substract the rest later
